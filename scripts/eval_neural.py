...
def main():
    ...
    # explicitly add a model name/path if auto-detect fails
    ...
    # look for ArcFace first, then Linear, then Legacy
    ...
    # if we add a specific name, use that
    if args.model_name_override:
        # strip extension if we accidentally accidentally added .pth
        clean_name = args.model_name_override.replace(".pth", "").replace(".json", "")
        # remove 'meta_' prefix if we added it accidentally also
